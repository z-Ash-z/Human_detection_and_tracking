Please send your questions to the \href{http://groups.google.com/group/googlemock}{\texttt{ googlemock}} discussion group. If you need help with compiler errors, make sure you have tried Google Mock Doctor first.\hypertarget{md_vendor_googletest_googlemock_docs_FrequentlyAskedQuestions_autotoc_md182}{}\doxysubsection{When I call a method on my mock object, the method for the real object is invoked instead.  What\textquotesingle{}s the problem?}\label{md_vendor_googletest_googlemock_docs_FrequentlyAskedQuestions_autotoc_md182}
In order for a method to be mocked, it must be {\itshape virtual}, unless you use the \href{CookBook.md\#mocking-nonvirtual-methods}{\texttt{ high-\/perf dependency injection technique}}.\hypertarget{md_vendor_googletest_googlemock_docs_FrequentlyAskedQuestions_autotoc_md183}{}\doxysubsection{I wrote some matchers.  After I upgraded to a new version of Google Mock, they no longer compile.  What\textquotesingle{}s going on?}\label{md_vendor_googletest_googlemock_docs_FrequentlyAskedQuestions_autotoc_md183}
...
For more information, you can read these \href{CookBook.md\#writing-new-monomorphic-matchers}{\texttt{ two}} \href{CookBook.md\#writing-new-polymorphic-matchers}{\texttt{ recipes}} from the cookbook. As always, you are welcome to post questions on {\ttfamily googlemock@googlegroups.\+com} if you need any help.\hypertarget{md_vendor_googletest_googlemock_docs_FrequentlyAskedQuestions_autotoc_md184}{}\doxysubsection{When using Google Mock, do I have to use Google Test as the testing framework?  I have my favorite testing framework and don\textquotesingle{}t want to switch.}\label{md_vendor_googletest_googlemock_docs_FrequentlyAskedQuestions_autotoc_md184}
Google \mbox{\hyperlink{classMock}{Mock}} works out of the box with Google Test. However, it\textquotesingle{}s easy to configure it to work with any testing framework of your choice. \href{ForDummies.md\#using-google-mock-with-any-testing-framework}{\texttt{ Here}} is how.\hypertarget{md_vendor_googletest_googlemock_docs_FrequentlyAskedQuestions_autotoc_md185}{}\doxysubsection{How am I supposed to make sense of these horrible template errors?}\label{md_vendor_googletest_googlemock_docs_FrequentlyAskedQuestions_autotoc_md185}
...
Or you can run {\ttfamily gmd} and copy-\/n-\/paste gcc\textquotesingle{}s error messages to it.\hypertarget{md_vendor_googletest_googlemock_docs_FrequentlyAskedQuestions_autotoc_md186}{}\doxysubsection{Can I mock a variadic function?}\label{md_vendor_googletest_googlemock_docs_FrequentlyAskedQuestions_autotoc_md186}
...
Ellipsis arguments are inherited from C and not really a C++ feature. They are unsafe to use and don\textquotesingle{}t work with arguments that have constructors or destructors. Therefore we recommend to avoid them in C++ as much as possible.\hypertarget{md_vendor_googletest_googlemock_docs_FrequentlyAskedQuestions_autotoc_md187}{}\doxysubsection{M\+S\+V\+C gives me warning C4301 or C4373 when I define a mock method with a const parameter.  Why?}\label{md_vendor_googletest_googlemock_docs_FrequentlyAskedQuestions_autotoc_md187}
...
\end{DoxyCode}
\hypertarget{md_vendor_googletest_googlemock_docs_FrequentlyAskedQuestions_autotoc_md188}{}\doxysubsection{I have a huge mock class, and Microsoft Visual C++ runs out of memory when compiling it.  What can I do?}\label{md_vendor_googletest_googlemock_docs_FrequentlyAskedQuestions_autotoc_md188}
We\textquotesingle{}ve noticed that when the {\ttfamily /clr} compiler flag is used, Visual C++ uses 5$\sim$6 times as much memory when compiling a mock class. We suggest to avoid {\ttfamily /clr} when compiling native C++ mocks.\hypertarget{md_vendor_googletest_googlemock_docs_FrequentlyAskedQuestions_autotoc_md189}{}\doxysubsection{I can\textquotesingle{}t figure out why Google Mock thinks my expectations are not satisfied.  What should I do?}\label{md_vendor_googletest_googlemock_docs_FrequentlyAskedQuestions_autotoc_md189}
You might want to run your test with {\ttfamily -\/-\/gmock\+\_\+verbose=info}. This flag lets Google \mbox{\hyperlink{classMock}{Mock}} print a trace of every mock function call it receives. By studying the trace, you\textquotesingle{}ll gain insights on why the expectations you set are not met.\hypertarget{md_vendor_googletest_googlemock_docs_FrequentlyAskedQuestions_autotoc_md190}{}\doxysubsection{How can I assert that a function is N\+E\+V\+E\+R called?}\label{md_vendor_googletest_googlemock_docs_FrequentlyAskedQuestions_autotoc_md190}

\begin{DoxyCode}{0}
\DoxyCodeLine{EXPECT\_CALL(foo, Bar(\_))}
\DoxyCodeLine{    .Times(0);}
\end{DoxyCode}
\hypertarget{md_vendor_googletest_googlemock_docs_FrequentlyAskedQuestions_autotoc_md191}{}\doxysubsection{I have a failed test where Google Mock tells me T\+W\+I\+C\+E that a particular expectation is not satisfied.  Isn\textquotesingle{}t this redundant?}\label{md_vendor_googletest_googlemock_docs_FrequentlyAskedQuestions_autotoc_md191}
...
By changing {\ttfamily $\sim$\+Base()} to virtual, {\ttfamily $\sim$\+Derived()} will be correctly called when {\ttfamily delete p} is executed, and the heap checker will be happy.\hypertarget{md_vendor_googletest_googlemock_docs_FrequentlyAskedQuestions_autotoc_md193}{}\doxysubsection{The \char`\"{}newer expectations override older ones\char`\"{} rule makes writing expectations awkward.  Why does Google Mock do that?}\label{md_vendor_googletest_googlemock_docs_FrequentlyAskedQuestions_autotoc_md193}
...
\end{DoxyEnumerate}\hypertarget{md_vendor_googletest_googlemock_docs_FrequentlyAskedQuestions_autotoc_md197}{}\doxysubsection{My code calls a static/global function.  Can I mock it?}\label{md_vendor_googletest_googlemock_docs_FrequentlyAskedQuestions_autotoc_md197}
...
This Google Testing Blog \href{http://googletesting.blogspot.com/2008/06/defeat-static-cling.html}{\texttt{ post}} says it excellently. Check it out.\hypertarget{md_vendor_googletest_googlemock_docs_FrequentlyAskedQuestions_autotoc_md198}{}\doxysubsection{My mock object needs to do complex stuff.  It\textquotesingle{}s a lot of pain to specify the actions.  Google Mock sucks!}\label{md_vendor_googletest_googlemock_docs_FrequentlyAskedQuestions_autotoc_md198}
...
See this \href{CookBook.md\#mocking_side_effects}{\texttt{ recipe}} for more details and an example.\hypertarget{md_vendor_googletest_googlemock_docs_FrequentlyAskedQuestions_autotoc_md202}{}\doxysubsection{My question is not in your F\+A\+Q!}\label{md_vendor_googletest_googlemock_docs_FrequentlyAskedQuestions_autotoc_md202}
